...
| La clasificación por área filtra en la vista del dashboard, no en RLS de `trainings` | Endurecer RLS rompería el check-in por QR de alguien de otra área que sí asistió a la sesión. El dashboard es donde la gente busca qué le falta; filtrar ahí cumple el objetivo sin romper la asistencia. |
| Una capacitación **sin** áreas es para todo el personal | Es el valor por omisión seguro: el error de no clasificar hace que la vea de más gente, no que se le esconda a quien le tocaba. Además las capacitaciones que ya existían no necesitaron reclasificarse. |
